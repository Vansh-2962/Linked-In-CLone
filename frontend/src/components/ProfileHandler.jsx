...
const ProfileHandler = ({ userData, isOwnProfile, onSave }) => {
  ...
  const { data: connectionStatus, refetch: refetchConnectionStatus } = useQuery(
    {
      ...
      queryFn: () => axios.get(`/connections/status/${userData._id}`),
  ...
  const { mutate: sendConnecitonRequest } = useMutation({
    mutationFn: (userId) => axios.post(`/connections/request/${userId}`),
    onSuccess: () => {
  ...
  const { mutate: rejectRequest } = useMutation({
    mutationFn: (requestId) => {
      axios.get(`/connections/reject/${requestId}`);
    },
    onSuccess: () => {
  ...
  const { mutate: removeConnection } = useMutation({
    mutationFn: (userId) => axios.delete(`/connections/${userId}`),
    onSuccess: () => {
  ...
  const renderConnectionButton = () => {
    ...
    switch (getConnectionState) {
      ...
      default:
        return (
          <button className="bg-secondary hover:bg-blue-600 text-white py-2 px-4 rounded-full transition duration-300 flex items-center justify-center">
      ...
      <div className="p-4">
        ...
        <div className="text-center mb-4">
          ...
          <div className="flex items-start justify-center mt-2">
            <MapPin size={18} className="text-gray-500 mr-1 mt-[0.3rem]" />
            {isEditing ? (
              <input
                type="text"
        ...
        {isOwnProfile ? (
          isEditing ? (
            <button
              className="w-full bg-secondary text-white py-2 px-4 rounded-full hover:bg-blue-600 transition duration-300"
              onClick={handleSave}
            >
              Save Profile
            </button>
